	...
	IN PRTMP_ADAPTER pAd)
{
	...
	AsicSendCommandToMcu(pAd, CD_CHECKOUT_MCU_CMD, 0xff, 0x00, 0x00);
	/* Debug */
	...
	IN PSTRING			arg)
{
...
#ifdef CARRIER_DETECTION_FIRMWARE_SUPPORT
	{
		...
		AsicSendCommandToMcu(pAd, CD_ONOFF_MCU_CMD, 0xff, 0x01, 0x00);
	}
#endif /* CARRIER_DETECTION_FIRMWARE_SUPPORT */
	return TRUE;
}
...
{		
	...
	if (pAd->CommonCfg.CarrierDetect.Enable)
	{
		...
		if (pAd->infType == RTMP_DEV_INF_PCIE)
			AsicSendCommandToMcu(pAd, CD_INT_POLLING_CMD, 0xff, 0x01, 0x00);
#endif /* CARRIER_DETECTION_FIRMWARE_SUPPORT */	
	}
}
...
VOID CarrierDetectionStop(IN PRTMP_ADAPTER	pAd)
{
	...
	AsicSendCommandToMcu(pAd, CD_ONOFF_MCU_CMD, 0xff, 0x00, 0x00);
#endif /* CARRIER_DETECTION_FIRMWARE_SUPPORT */
	return;
}
